...
namespace AccountingServer.BLL
{
    ...
    public class ReimbursementReport
    {
        ...
        ///     报表数据
        /// </summary>
        private readonly IEnumerable<ReportItem> m_Report;
        ...
        {
            {
                ...
                foreach (var detail in lst)
                    yield return
                        new ReportItem
                            {
                                Title = TitleManager.GetTitleName(detail.Title),
                                Content = "（移动电源零部件）",
            ...
            {
                var lst = helper.GetBalancesAcrossContent(
            ...
            {
                var lst = helper.GetBalancesAcrossContent(
                                                          new Balance { Title = 6602, SubTitle = 03 },
                                                          startDate,
                                                          endDate);
                foreach (var detail in lst)
            ...
            {
                var lst = helper.GetBalancesAcrossContent(
                                                          new Balance { Title = 6602, SubTitle = 06 },
                                                          startDate,
                                                          endDate);
                foreach (var detail in lst)
                    switch (detail.Content)
