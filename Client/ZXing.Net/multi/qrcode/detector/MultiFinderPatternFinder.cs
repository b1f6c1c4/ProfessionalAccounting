...
namespace ZXing.Multi.QrCode.Internal
{
    ...
    internal sealed class MultiFinderPatternFinder : FinderPatternFinder
    {
        ...
        private sealed class ModuleSizeComparator : IComparer<FinderPattern>
        {
            ...
            {
                var value = center2.EstimatedModuleSize - center1.EstimatedModuleSize;
        ...
        /// </summary>
        /// <returns>
        ///     the 3 best <see cref="FinderPattern" />s from our list of candidates. The "best" are
        ...
        private FinderPattern[][] selectMutipleBestPatterns()
        {
            ...
            var size = possibleCenters.Count;
          ...
          */
            ...
            var results = new List<FinderPattern[]>(); // holder for the results
            ...
            for (var i1 = 0; i1 < (size - 2); i1++)
            {
                ...
                for (var i2 = i1 + 1; i2 < (size - 1); i2++)
                {
                    ...
                    for (var i3 = i2 + 1; i3 < size; i3++)
                    {
                        ...
                        var dA = ResultPoint.distance(info.TopLeft, info.BottomLeft);
                        var dC = ResultPoint.distance(info.TopRight, info.BottomLeft);
                        var dB = ResultPoint.distance(info.TopLeft, info.TopRight);

                        // Check the sizes
                        var estimatedModuleCount = (dA + dB) / (p1.EstimatedModuleSize * 2.0f);
        ...
        public FinderPatternInfo[] findMulti(IDictionary<DecodeHintType, object> hints)
        {
            ...
            var patternInfo = selectMutipleBestPatterns();
            if (patternInfo == null)
