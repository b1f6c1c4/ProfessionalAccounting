...
namespace ZXing.OneD
{
    ...
    public sealed class MSIReader : OneDReader
    {
        ...
        public override Result decodeRow(int rowNumber, BitArray row, IDictionary<DecodeHintType, object> hints)
        {
            ...
            var start = findStartPattern(row, counters);
            if (start == null)
                return null;
            ...
            if (usingCheckDigit)
            {
                ...
                var checkSum = CalculateChecksumLuhn(resultStringWithoutChecksum);
                if ((char)(checkSum + 48) != resultString[resultStringWithoutChecksum.Length])
                    return null;
            }

            var left = (start[1] + start[0]) / 2.0f;
            var right = (nextStart + lastStart) / 2.0f;
