...
namespace ZXing.OneD
{
    ...
    ///         At the moment it reads length 6, 8, 10, 12, 14, 16, 18, 20, 24, 44 and 48 as these have appeared "in the wild".
    ///         Not all
    ...
    public sealed class ITFReader : OneDReader
    {
        ...
        public override Result decodeRow(int rowNumber, BitArray row, IDictionary<DecodeHintType, object> hints)
        {
            ...
            var startRange = decodeStart(row);
            if (startRange == null)
                return null;

            var endRange = decodeEnd(row);
            if (endRange == null)
                return null;
            ...
            if (hints != null &&
                hints.ContainsKey(DecodeHintType.ALLOWED_LENGTHS))
            {
            ...
            var lengthOK = length > LARGEST_DEFAULT_ALLOWED_LENGTH;
            if (!lengthOK)
            {
                foreach (var allowedLength in allowedLengths)
                {
                                         ...
                                         StringBuilder resultString)
        {
            ...
            while (payloadStart < payloadEnd)
            {
                ...
                foreach (var counterDigit in counterDigitPair)
        ...
        {
            var endStart = skipWhiteSpace(row);
            if (endStart < 0)
                return null;

            var startPattern = findGuardPattern(row, endStart, START_PATTERN);
        ...
        {
            var quietCount = narrowLineWidth * 10; // expect to find this many pixels of quiet zone
        ...
        private int[] decodeEnd(BitArray row)
        {
            ...
            if (endStart < 0)
                return null;
            var endPattern = findGuardPattern(row, endStart, END_PATTERN_REVERSED);
            if (endPattern == null)
            {
                row.reverse();
                return null;
