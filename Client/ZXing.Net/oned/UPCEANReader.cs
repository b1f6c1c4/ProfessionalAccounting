...
namespace ZXing.OneD
{
    ...
    public abstract class UPCEANReader : OneDReader
    {
        ...
        internal static int[] findStartGuardPattern(BitArray row)
        {
            ...
            int[] startRange = null;
            var nextStart = 0;
            var counters = new int[START_END_PATTERN.Length];
            while (!foundStart)
            {
                for (var idx = 0; idx < START_END_PATTERN.Length; idx++)
                    counters[idx] = 0;
                                        ...
                                        IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (extensionResult != null)
            {
                ...
                var extensionLength = extensionResult.Text.Length;
                var allowedExtensions = hints != null && hints.ContainsKey(DecodeHintType.ALLOWED_EAN_EXTENSIONS)
                                            ? (int[])hints[DecodeHintType.ALLOWED_EAN_EXTENSIONS]
                                            : null;
                if (allowedExtensions != null)
                {
                    var valid = false;
                    foreach (var length in allowedExtensions)
            ...
            {
                var countryID = eanManSupport.lookupCountryIdentifier(resultString);
                if (countryID != null)
        ...
        /// <returns>
        ///     horizontal offset of first pixel after the "middle" that was decoded or -1 if decoding could not complete
        ///     successfully
        /// </returns>
