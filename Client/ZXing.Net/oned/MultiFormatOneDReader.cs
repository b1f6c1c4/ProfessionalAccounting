...
namespace ZXing.OneD
{
    ...
    public sealed class MultiFormatOneDReader : OneDReader
    {
        ...
        {
            var possibleFormats = hints == null || !hints.ContainsKey(DecodeHintType.POSSIBLE_FORMATS)
                                      ? null
            ...
            if (possibleFormats != null)
            {
                ...
                if (possibleFormats.Contains(BarcodeFormat.MSI))
                {
                    ...
                    var useMsiCheckDigit = (hints.ContainsKey(DecodeHintType.ASSUME_MSI_CHECK_DIGIT)
                                                ? (bool)hints[DecodeHintType.ASSUME_MSI_CHECK_DIGIT]
                                                : false);
                    readers.Add(new MSIReader(useMsiCheckDigit));
                }
                if (possibleFormats.Contains(BarcodeFormat.CODE_39) ||
                    possibleFormats.Contains(BarcodeFormat.All_1D))
                {
                    var useCode39CheckDigit = hints.ContainsKey(DecodeHintType.ASSUME_CODE_39_CHECK_DIGIT) &&
                                              (bool)hints[DecodeHintType.ASSUME_CODE_39_CHECK_DIGIT];
                    var useCode39ExtendedMode = hints.ContainsKey(DecodeHintType.USE_CODE_39_EXTENDED_MODE) &&
            ...
            if (readers.Count == 0)
            {
                var useCode39CheckDigit = hints != null && hints.ContainsKey(DecodeHintType.ASSUME_CODE_39_CHECK_DIGIT) &&
                                          (bool)hints[DecodeHintType.ASSUME_CODE_39_CHECK_DIGIT];
                var useCode39ExtendedMode = hints != null && hints.ContainsKey(DecodeHintType.USE_CODE_39_EXTENDED_MODE) &&
                                            (bool)hints[DecodeHintType.USE_CODE_39_EXTENDED_MODE];
                // MSI needs to be activated explicit
