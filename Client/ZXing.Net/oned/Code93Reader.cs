...
namespace ZXing.OneD
{
    ...
    public sealed class Code93Reader : OneDReader
    {
        ...
        public override Result decodeRow(int rowNumber, BitArray row, IDictionary<DecodeHintType, object> hints)
        {
            ...
            var start = findAsteriskPattern(row);
            if (start == null)
                return null;
            ...
            do
            {
                ...
                var pattern = toPattern(counters);
                if (pattern < 0)
                    return null;
            ...
            var resultString = decodeExtended(decodeRowResult);
        ...
        private static int toPattern(int[] counters)
        {
            ...
            for (var i = 0; i < max; i++)
            {
                var scaledShifted = (counters[i] << INTEGER_MATH_SHIFT) * 9 / sum;
                var scaledUnshifted = scaledShifted >> INTEGER_MATH_SHIFT;
                if ((scaledShifted & 0xFF) > 0x7F)
                    scaledUnshifted++;
                if (scaledUnshifted < 1 ||
                    scaledUnshifted > 4)
                    return -1;
                if ((i & 0x01) == 0)
                    for (var j = 0; j < scaledUnshifted; j++)
