    ...
    public abstract class OneDReader : Reader
    {
        ...
        public virtual Result decode(BinaryBitmap image, IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (result == null)
            {
                ...
                var tryHarderWithoutRotation = hints != null &&
                                               hints.ContainsKey(DecodeHintType.TRY_HARDER_WITHOUT_ROTATION);
                if (tryHarder &&
                    !tryHarderWithoutRotation &&
                    image.RotateSupported)
                {
                    ...
                    if (points != null)
                    {
                        var height = rotatedImage.Height;
        ...
        private Result doDecode(BinaryBitmap image, IDictionary<DecodeHintType, object> hints)
        {
            ...
            for (var x = 0; x < maxLines; x++)
            {
                ...
                for (var attempt = 0; attempt < 2; attempt++)
                {
                    ...
                    // Look for a barcode
                    var result = decodeRow(rowNumber, row, hints);
                    if (result == null)
                        continue;
        ...
        protected static bool recordPattern(BitArray row,
                                            ...
                                            int numCounters)
        {
            for (var idx = 0; idx < numCounters; idx++)
                counters[idx] = 0;
            var end = row.Size;
            if (start >= end)
                return false;
