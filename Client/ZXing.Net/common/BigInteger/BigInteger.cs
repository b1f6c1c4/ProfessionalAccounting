...
namespace BigIntegerLibrary
{
        ...
        IEquatable<BigInteger>, IComparable, IComparable<BigInteger>
    {
        ...
        /// </summary>
        private readonly DigitContainer digits;
        ...
        ///     The byte array's content exceeds the maximum size of a BigInteger
        ///     exception
        /// </exception>
        public BigInteger(byte[] byteArray)
        {
            ...
            {
                int currentDigit = byteArray[i];
                if (i + 1 < byteArray.Length)
                {
                    currentDigit <<= 8;
                    currentDigit += byteArray[i + 1];
                }

                digits[size++] = currentDigit;
            }

            var reducible = true;
            while ((size - 1 > 0) && reducible)
                if (digits[size - 1] == 0)
                    size--;
                else
                    reducible = false;
        ...
        {
            var signValue = (bool)info.GetValue("sign", typeof(bool));
            if (signValue)
                sign = Sign.Positive;
            else
                sign = Sign.Negative;
            ...
            int i;
            for (i = 0; i < size; i++)
                digits[i] = (long)(info.GetValue(
                                                 string.Format("{0}{1}", "d_", i),
                                                 typeof(ushort)));
        }
#endif
    ...
    /// <permission cref="System.Security.PermissionSet">Public access</permission>
        ...
        /// <returns>
        ///     True if the specified BigInteger is equal to the current BigInteger,
        ///     false otherwise
        /// </returns>
        ...
        /// <returns>
        ///     True if the specified System.Object is equal to the current BigInteger,
        ///     false otherwise
        /// </returns>
        ...
        {
            var res = new Base10BigInteger();
            var currentPower = new Base10BigInteger(1);
        ...
        /// <returns>
        ///     -1 if the current instance is smaller than the given BigInteger,
        ///     0 if the two are equal, 1 otherwise
        /// </returns>
        ...
        /// <returns>
        ///     -1 if the current instance is smaller than the given object,
        ///     0 if the two are equal, 1 otherwise
        /// </returns>
        ...
        public static BigInteger IntegerSqrt(BigInteger n)
        {
            if (n.sign == Sign.Negative)
                throw new BigIntegerException(
                    "Cannot compute the integer square root of a negative number.",
                    null);

            var oldValue = new BigInteger(0);
            var newValue = new BigInteger(n);
                                                   ...
                                                   out BigInteger u, out BigInteger v)
        {
            ...
            var u1 = new BigInteger();
            var u2 = new BigInteger(1);
            var v1 = new BigInteger(1);
            var v2 = new BigInteger();
            BigInteger q, r;
        ...
        public static BigInteger ModularInverse(BigInteger a, BigInteger n)
        {
            ...
            if (Gcd(a, n) != One)
                throw new BigIntegerException(
                    "Cannot obtain the modular inverse of a number that is not coprime with the modulus.",
                    null);
...
*/
        ...
        private class DigitContainer
        {
            ...
            private const int ChunkCount = MaxSize >> ChunkSizeDivisionShift;

            public DigitContainer() { digits = new long[ChunkCount][]; }
