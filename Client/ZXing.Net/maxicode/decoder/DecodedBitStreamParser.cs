...
namespace ZXing.Maxicode.Internal
{
    ...
    internal static class DecodedBitStreamParser
    {
        ...
        private static readonly String[] SETS =
            {
                "\nABCDEFGHIJKLMNOPQRSTUVWXYZ" + ECI + FS + GS + RS + NS + ' ' + PAD + "\"#$%&'()*+,-./0123456789:" +
                SHIFTB + SHIFTC + SHIFTD + SHIFTE + LATCHB,
                "`abcdefghijklmnopqrstuvwxyz" + ECI + FS + GS + RS + NS + '{' + PAD + "}~\u007F;<=>?[\\]^_ ,./:@!|" +
        ...
        internal static DecoderResult decode(byte[] bytes, int mode)
        {
            ...
            switch (mode)
            {
                ...
                case 3:
                    ...
                    if (mode == 2)
                    {
                        var pc = getPostCode2(bytes);
                        var df = "0000000000".Substring(0, getPostCode2Length(bytes));
                        postcode = pc.ToString(df);
                    }
                    else
                        postcode = getPostCode3(bytes);
                    var country = getCountry(bytes).ToString(THREE_DIGITS);
                    var service = getServiceClass(bytes).ToString(THREE_DIGITS);
            ...
            for (var i = start; i < start + len; i++)
            {
                ...
                switch (c)
                {
                    ...
                    case THREESHIFTA:
                        ...
                        break;
                    case NS:
                        var nsval = (bytes[++i] << 24) + (bytes[++i] << 18) + (bytes[++i] << 12) + (bytes[++i] << 6) +
                                    bytes[++i];
