...
namespace ZXing.Datamatrix.Internal
{
    ...
    internal static class DecodedBitStreamParser
    {
                ...
                for (var i = 0; i < 3; i++)
                {
                    ...
                    switch (shift)
                    {
                        case 0:
                            ...
                            {
                                var c40char = C40_BASIC_SET_CHARS[cValue];
                                if (upperShift)
                                {
                        ...
                        case 2:
                            ...
                            {
                                var c40char = C40_SHIFT2_SET_CHARS[cValue];
                                if (upperShift)
                                {
                                    result.Append((char)(c40char + 128));
                                    upperShift = false;
                ...
                for (var i = 0; i < 3; i++)
                {
                    ...
                    switch (shift)
                    {
                        case 0:
                            ...
                            {
                                var textChar = TEXT_BASIC_SET_CHARS[cValue];
                                if (upperShift)
                                {
                        ...
                        case 3:
                            ...
                            {
                                var textChar = TEXT_SHIFT3_SET_CHARS[cValue];
                                if (upperShift)
                                {
                                    result.Append((char)(textChar + 128));
                                    upperShift = false;
                                                 ...
                                                 IList<byte[]> byteSegments)
        {
            ...
            var codewordPosition = 1 + bits.ByteOffset; // position is 1-indexed
            var d1 = unrandomize255State(bits.readBits(8), codewordPosition++);
            int count;
            if (d1 == 0)
