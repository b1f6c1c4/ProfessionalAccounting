...
namespace ZXing.Datamatrix.Encoder
{
    ...
    public static class ErrorCorrection
    {
        ...
        public static String encodeECC200(String codewords, SymbolInfo symbolInfo)
        {
            ...
            if (blockCount == 1)
            {
                var ecc = createECCBlock(codewords, symbolInfo.errorCodewords);
                sb.Append(ecc);
            }
            else
            {
                sb.Length = sb.Capacity;
                var dataSizes = new int[blockCount];
                var errorSizes = new int[blockCount];
                var startPos = new int[blockCount];
                ...
                for (var block = 0; block < blockCount; block++)
                {
                    ...
                    for (var d = block; d < symbolInfo.dataCapacity; d += blockCount)
                        temp.Append(codewords[d]);
                    var ecc = createECCBlock(temp.ToString(), errorSizes[block]);
                    var pos = 0;
        ...
        private static String createECCBlock(String codewords, int start, int len, int numECWords)
        {
            ...
            var ecc = new char[numECWords];
            for (var i = 0; i < numECWords; i++)
                ecc[i] = (char)0;
            for (var i = start; i < start + len; i++)
            {
                var m = ecc[numECWords - 1] ^ codewords[i];
            ...
            var eccReversed = new char[numECWords];
            for (var i = 0; i < numECWords; i++)
