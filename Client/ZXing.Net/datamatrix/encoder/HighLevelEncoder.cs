...
namespace ZXing.Datamatrix.Encoder
{
    ...
    internal static class HighLevelEncoder
    {
      ...
      */
        ...
        /// <param name="msg">the message</param>
        /// <param name="shape">
        ///     requested shape. May be {@code SymbolShapeHint.FORCE_NONE},{@code SymbolShapeHint.FORCE_SQUARE} or
                                             ...
                                             int defaultEncodation)
        {
            ...
            if (msg.StartsWith(MACRO_05_HEADER) &&
            ...
            var encodingMode = defaultEncodation; //Default mode
            switch (encodingMode)
            {
            ...
            var len = context.Codewords.Length;
            context.updateSymbolInfo();
            var capacity = context.SymbolInfo.dataCapacity;
            if (len < capacity)
                if (encodingMode != Encodation.ASCII &&
                    encodingMode != Encodation.BASE256)
                    context.writeCodeword('\u00fe'); //Unlatch (254)
            //Padding
            var codewords = context.Codewords;
        ...
        internal static int lookAheadTest(String msg, int startpos, int currentMode)
        {
            ...
            while (true)
            {
                ...
                if ((startpos + charsProcessed) == msg.Length)
                {
                    ...
                    if (minCount == 1 &&
                        mins[Encodation.BASE256] > 0)
                        return Encodation.BASE256;
                    if (minCount == 1 &&
                        mins[Encodation.EDIFACT] > 0)
                        return Encodation.EDIFACT;
                    if (minCount == 1 &&
                        mins[Encodation.TEXT] > 0)
                        return Encodation.TEXT;
                    if (minCount == 1 &&
                        mins[Encodation.X12] > 0)
                        return Encodation.X12;
                    return Encodation.C40;
                }

                var c = msg[startpos + charsProcessed];
                charsProcessed++;
                ...
                if (charsProcessed >= 4)
                {
                    ...
                    if (intCharCounts[Encodation.BASE256] < intCharCounts[Encodation.ASCII]
                        ||
                        (mins[Encodation.C40] + mins[Encodation.TEXT] + mins[Encodation.X12] + mins[Encodation.EDIFACT]) ==
                        0)
                        return Encodation.BASE256;
                    if (minCount == 1 &&
                        mins[Encodation.EDIFACT] > 0)
        ...
        internal static bool isDigit(char ch) { return ch >= '0' && ch <= '9'; }

        internal static bool isExtendedASCII(char ch) { return ch >= 128 && ch <= 255; }
        ...
        internal static bool isNativeEDIFACT(char ch) { return ch >= ' ' && ch <= '^'; }
