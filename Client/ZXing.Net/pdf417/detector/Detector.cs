...
namespace ZXing.PDF417.Internal
{
    ...
    public sealed class Detector
    {
        ...
        /// <summary>
        ///     A PDF471 barcode should have at least 3 rows, with each row being >= 3 times the module width. Therefore it should
        ///     be at least
        ...
        /// <param name="multiple">
        ///     if true, then the image is searched for multiple codes. If false, then at most one code will be
        ///     found and returned
        /// </param>
        /// <returns>
        ///     <see cref="PDF417DetectorResult" /> encapsulating results of detecting a PDF417 code
        /// </returns>
        public static PDF417DetectorResult detect(BinaryBitmap image, IDictionary<DecodeHintType, object> hints,
                                                  bool multiple)
        {
            ...
            var bitMatrix = image.BlackMatrix;
        ...
        /// <summary>
        ///     Detects PDF417 codes in an image. Only checks 0 degree rotation (so rotate the matrix and check again outside of
        ///     this method)
        /// </summary>
        /// <param name="multiple">
        ///     multiple if true, then the image is searched for multiple codes. If false, then at most one code
        ...
        private static List<ResultPoint[]> detect(bool multiple, BitMatrix bitMatrix)
        {
            ...
            while (row < bitMatrix.Height)
            {
                    ...
                    vertices[3] == null)
                {
                    ...
                    column = 0;
                    foreach (var barcodeCoordinate in barcodeCoordinates)
                    {
            ...
            int[] pattern)
        {
            ...
            var found = false;
            var counters = new int[pattern.Length];
            for (; startRow < height; startRow += ROW_STEP)
            {
                ...
                if (loc != null)
                {
                    while (startRow > 0)
                    {
                        var previousRowLoc = findGuardPattern(
                                                              matrix,
                                                              startColumn,
                                                              --startRow,
                                                              width,
        ...
        /// <summary>
        ///     Finds the guard pattern.  Uses System.Linq.Enumerable.Repeat to fill in counters.  This might be a performance
        ///     issue?
        /// </summary>
