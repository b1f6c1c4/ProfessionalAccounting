    ...
    public sealed class BoundingBox
    {
        ...
        public BoundingBox addMissingRows(int missingStartRows, int missingEndRows, bool isLeft)
        {
            ...
            if (missingStartRows > 0)
            {
                ...
                var newMinY = (int)top.Y - missingStartRows;
                if (newMinY < 0)
                    newMinY = 0;
                // TODO use existing points to better interpolate the new x positions
                var newTop = new ResultPoint(top.X, newMinY);
                if (isLeft)
                    newTopLeft = newTop;
                else
                    newTopRight = newTop;
            ...
            if (missingEndRows > 0)
            {
                ...
                var newMaxY = (int)bottom.Y + missingEndRows;
                if (newMaxY >= image.Height)
                    newMaxY = image.Height - 1;
                // TODO use existing points to better interpolate the new x positions
                var newBottom = new ResultPoint(bottom.X, newMaxY);
                if (isLeft)
