...
namespace ZXing.PDF417.Internal
{
    ...
    internal static class PDF417HighLevelEncoder
    {
        ...
        internal static String[] DEFAULT_ENCODING_NAMES = {"CP437", "IBM437"};

        static PDF417HighLevelEncoder()
        {
            //Construct inverse lookups
            for (var idx = 0; idx < MIXED.Length; idx++)
                MIXED[idx] = -1;
            for (sbyte i = 0; i < TEXT_MIXED_RAW.Length; i++)
            {
                var b = TEXT_MIXED_RAW[i];
                if (b > 0)
                    MIXED[b] = i;
            }
            for (var idx = 0; idx < PUNCTUATION.Length; idx++)
                PUNCTUATION[idx] = -1;
            for (sbyte i = 0; i < TEXT_PUNCTUATION_RAW.Length; i++)
            {
                var b = TEXT_PUNCTUATION_RAW[i];
                if (b > 0)
                    PUNCTUATION[b] = i;
            }
        }
        ...
        internal static String encodeHighLevel(String msg, Compaction compaction, Encoding encoding, bool disableEci)
        {
            ...
            if (encoding != null &&
                !disableEci &&
                !Contains(DEFAULT_ENCODING_NAMES, encoding.WebName))
            {
                var eci = CharacterSetECI.getCharacterSetECIByName(encoding.WebName);
                if (eci != null)
            ...
            {
                var encodingMode = TEXT_COMPACTION; //Default mode, see 4.4.2.1
                while (p < len)
                {
                    var n = determineConsecutiveDigitCount(msg, p);
                    if (n >= 13)
                    {
                        sb.Append((char)LATCH_TO_NUMERIC);
                    ...
                    {
                        var t = determineConsecutiveTextCount(msg, p);
                        if (t >= 5 ||
                            n == len)
                        {
                        ...
                        else
                        {
                            if (bytes == null)
                                bytes = toBytes(msg, encoding);
                            var b = determineConsecutiveBinaryCount(msg, bytes, p);
                            if (b == 0)
                                b = 1;
                            if (b == 1 &&
                                encodingMode == TEXT_COMPACTION)
        ...
        private static byte[] toBytes(String msg, Encoding encoding)
        {
            ...
            if (encoding == null)
            {
                ...
                {
                    var encodingName = DEFAULT_ENCODING_NAMES[index];
                    try
                    {
        ...
        {
            if (count == 1 &&
                startmode == TEXT_COMPACTION)
                sb.Append((char)SHIFT_TO_BYTE);
            else
            {
                var sixpack = ((count % 6) == 0);
                if (sixpack)
...
#else
            var idx = 0;
            var tmp = new StringBuilder(count / 3 + 1);
            var num900 = new BigInteger(900);
            var num0 = new BigInteger(0);
            while (idx < count - 1)
            {
                tmp.Length = 0;
                var len = Math.Min(44, count - idx);
                var part = '1' + msg.Substring(startpos + idx, len);
                var bigint = BigInteger.Parse(part);
                do
                {
                    var c = BigInteger.Modulo(bigint, num900);
        ...
        private static bool isAlphaUpper(char ch) { return ch == ' ' || (ch >= 'A' && ch <= 'Z'); }

        private static bool isAlphaLower(char ch) { return ch == ' ' || (ch >= 'a' && ch <= 'z'); }

        private static bool isMixed(char ch) { return MIXED[ch] != -1; }

        private static bool isPunctuation(char ch) { return PUNCTUATION[ch] != -1; }
