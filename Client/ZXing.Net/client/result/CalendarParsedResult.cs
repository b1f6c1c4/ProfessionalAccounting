...
namespace ZXing.Client.Result
{
    ...
    public sealed class CalendarParsedResult : ParsedResult
    {
        ...
        private static readonly Regex DATE_TIME = new Regex(
            @"\A(?:" + "[0-9]{8}(T[0-9]{6}Z?)?" + @")\z"
#if !(SILVERLIGHT4 || SILVERLIGHT5 || NETFX_CORE || PORTABLE)
            ,
            RegexOptions.Compiled);
            ...
            : base(ParsedResultType.CALENDAR)
        {
            ...
            if (endString == null)
            {
                var durationMS = parseDurationMS(durationString);
        ...
        public bool isEndAllDay { get { return endAllDay; } }

        public String Location { get { return location; } }

        public String Organizer { get { return organizer; } }

        public String[] Attendees { get { return attendees; } }
        ...
        private static long parseDurationMS(String durationString)
        {
            ...
            if (!m.Success)
                return -1L;
            var durationMS = 0L;
            for (var i = 0; i < RFC2445_DURATION_FIELD_UNITS.Length; i++)
            {
                var fieldValue = m.Groups[i + 1].Value;
        ...
        private static string buildDateTimeFormat() { return "yyyyMMdd'T'HHmmss"; }
    }
}
