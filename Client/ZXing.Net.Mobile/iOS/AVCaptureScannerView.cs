...
namespace ZXing.Mobile
{
    public class AVCaptureScannerView : UIView, IZXingScanner<UIView>
    {
        ...
        private volatile bool foundResult;
        ...
        private bool SetupCaptureSession()
        {
            ...
            var dg = new CaptureDelegate(
                metaDataObjects =>
                {
                    ...
                    var rs = new Result(readableObj.StringValue, null, null, zxingFormat);
                ...
                ScanningOptions.PossibleFormats.Count > 0)
            {
                ...
                metadataOutput.MetadataObjectTypes = (from f in formats.Distinct()
                                                      select new NSString(f)).ToArray();
            }
            else
            ...
            if (previewLayer.RespondsToSelector(new Selector("connection")))
                if (UIDevice.CurrentDevice.CheckSystemVersion(7, 0))
                {
                    ...
                    previewLayer.Connection.VideoMinFrameDuration = new CMTime(1, 30);
        ...
        private string[] AVCaptureBarcodeFormatFromZXingBarcodeFormat(BarcodeFormat zxingBarcodeFormat)
        {
            var formats = new List<string>();
    ...
    internal class CaptureDelegate : AVCaptureMetadataOutputObjectsDelegate
    {
        public CaptureDelegate(Action<IEnumerable<AVMetadataObject>> onCapture) { OnCapture = onCapture; }
