    ...
    public class ZXingScannerView : UIView, IZXingScanner<UIView>
    {
        ...
        private void Setup(CGRect frame)
        {
            ...
            if (UseCustomOverlayView && CustomOverlayView != null)
                ...
                overlayView = new ZXingDefaultOverlayView(
                    new CGRect(0, 0, Frame.Width, Frame.Height),
                    TopText,
                    BottomText,
                    CancelButtonText,
                    FlashButtonText,
            ...
            if (overlayView != null)
            {
                ...
                overlayView.Frame = new CGRect(0, 0, Frame.Width, Frame.Height);
        ...
        private bool SetupCaptureSession()
        {
            ...
            var consideredResolutions = new Dictionary<NSString, CameraResolution>
                                            {
                                                {
                                                    AVCaptureSession.Preset352x288,
                                                    new CameraResolution {Width = 352, Height = 288}
                                                },
                                                {
                                                    AVCaptureSession.PresetMedium,
                                                    new CameraResolution {Width = 480, Height = 360}
                                                }, //480x360
                                                {
                                                    AVCaptureSession.Preset640x480,
                                                    new CameraResolution {Width = 640, Height = 480}
                                                },
                                                {
                                                    AVCaptureSession.Preset1280x720,
                                                    new CameraResolution {Width = 1280, Height = 720}
                                                }
                                            };
            ...
            foreach (var device in devices)
            {
                ...
                if (device.Position == AVCaptureDevicePosition.Back &&
                    (!options.UseFrontCameraIfAvailable.HasValue || !options.UseFrontCameraIfAvailable.Value))
            ...
            Console.WriteLine(
                              "PERF: Alloc AVCaptureVideoPreviewLayer took {0} ms.",
                              totalAVPreviewLayerAlloc.TotalMilliseconds);
            ...
            var barcodeReader = new BarcodeReader(
                ...
                img =>
                {
                    ...
                    if (ScanningOptions.AutoRotate.HasValue &&
                        ScanningOptions.AutoRotate.Value)
                        return src;
                ...
                null,
                null); //(p, w, h, f) => new RGBLuminanceSource(p, w, h, RGBLuminanceSource.BitmapFormat.Unknown));
            ...
            if (ScanningOptions.PossibleFormats != null &&
                ScanningOptions.PossibleFormats.Count > 0)
            {
            ...
            if (captureDevice.LockForConfiguration(out err))
            {
                ...
                if (UIDevice.CurrentDevice.CheckSystemVersion(7, 0) &&
                    captureDevice.AutoFocusRangeRestrictionSupported)
        ...
        public class OutputRecorder : AVCaptureVideoDataOutputSampleBufferDelegate
        {
            ...
            private readonly Action<UIImage> HandleImage;

            private DateTime lastAnalysis = DateTime.MinValue;
            private volatile bool working;

            [Export("captureOutput:didDropSampleBuffer:fromConnection:")]
            public void DidDropSampleBuffer(AVCaptureOutput captureOutput, CMSampleBuffer sampleBuffer,
        ...
        public void StartScanning(MobileBarcodeScanningOptions options, Action<Result> callback)
        {
            ...
            InvokeOnMainThread(
                               () =>
                               {
                                   ...
                                   {
                                       var simView = new UIView(new CGRect(0, 0, Frame.Width, Frame.Height));
                                       simView.BackgroundColor = UIColor.LightGray;
                                       simView.AutoresizingMask = UIViewAutoresizing.FlexibleWidth |
            ...
            var evt = OnScannerSetupComplete;
            if (evt != null)
                evt();
        }
        ...
        public void PauseAnalysis() { analyzing = false; }

        public void ResumeAnalysis() { analyzing = true; }
        ...
        public void ToggleTorch() { SetTorch(!IsTorchOn); }
