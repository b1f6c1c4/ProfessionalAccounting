...
namespace AccountingServer.Shell.Plugins.Interest
{
    ...
    internal abstract class InterestBase : PluginBase
    {
        ...
        public override IQueryResult Execute(string expr, IEntitiesSerializer serializer)
        {
            ...
            var loans = Accountant.RunGroupedQuery($"T{MajorTitle().AsTitle()}-\"\" ``rc").Items.Cast<ISubtotalRemark>()
                .ToList();
            var rmkObj =
                loans.Single(
                        b =>
            ...
            var content = rmkObj.Items.Cast<ISubtotalContent>().Single().Content;
            var rmk = rmkObj.Remark;

            if (!all && !endDate.HasValue ||
                endDate.HasValue)
            {
                var filter = $"T{MajorTitle().AsTitle()} {content.Quotation('\'')} {rmk.Quotation('"')}";
                var filter0 = $"T{MajorTitle().AsTitle()} {content.Quotation('\'')} {(rmk + "-利息").Quotation('"')}";
            ...
            ref double interestIntegral, DateTime? lastSettlement, DateTime finalDay)
        {
            ...
            foreach (var grp in
                Accountant
                    .RunVoucherQuery(
                        $"(T{MajorTitle().AsTitle()} {content.Quotation('\'')})*({rmk.Quotation('"')}+{(rmk + "-利息").Quotation('"')}) {rng.AsDateRange()}")
                    .GroupBy(v => v.Date)
