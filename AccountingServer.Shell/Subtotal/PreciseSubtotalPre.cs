...
internal class PreciseSubtotalPre : StringSubtotalVisitor
{
    ...
    public override async IAsyncEnumerable<string> Visit(ISubtotalVoucherRemark sub)
    {
        var prev = m_Path;
        m_Path = Merge(m_Path, sub.VoucherRemark);
        await foreach (var s in ShowSubtotal(sub))
            yield return s;
        m_Path = prev;
    }

    public override async IAsyncEnumerable<string> Visit(ISubtotalVoucherType sub)
    {
        var prev = m_Path;
        m_Path = Merge(m_Path, $"{sub.Type}");
        await foreach (var s in ShowSubtotal(sub))
            yield return s;
        m_Path = prev;
    }

    public override async IAsyncEnumerable<string> Visit(ISubtotalTitleKind sub)
    {
        var prev = m_Path;
        m_Path = Merge(m_Path, $"{sub.Kind}");
        await foreach (var s in ShowSubtotal(sub))
            yield return s;
        m_Path = prev;
    }
